...
class TransWindow(QWidget):
    def __init__(self):
        ...
        self.setStyleSheet(
            "QWidget { background-color: rgba(255,255,255, 5%); }")

    def mousePressEvent(self, QMouseEvent):
        self.hide()
        xd = QApplication.desktop().screenGeometry().x() - \
            QApplication.desktop().availableGeometry().x()
        yd = QApplication.desktop().screenGeometry().y() - \
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.arrow_icon = os.path.abspath(
            os.path.dirname(__file__)+"/cursor3.png")
    ...
    def capture(self):
        ...
        if not self.snap_shots.queue.empty():
            ...
            if(self.options.capture_area_sa.isChecked()):
                ...
                self.px2 = self.px
            self.px2.save(
                ('tmp_{}_{:'+self.fmt+'}.jpg').format(self.UID, self.capture_count), 'jpg')
            self.capture_count += 1

    def encode_video(self):
        fprefix = ('tmp_{}_').format(self.UID)
        fps = int(float(self.capture_count) /
                  (self.stop_capture_time-self.start_capture_time))
